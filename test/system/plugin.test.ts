...
test('run inspect() 0.13', async (t) => {
  ...
  t.equal(
    result.package.packageFormatVersion,
    'mvn:0.0.1',
    'correct package format version',
  );
...
test('run inspect() on bad project requiring user input', async (t) => {
  try {
    await plugin.inspect(
      path.join(__dirname, '..', 'fixtures', 'bad-project'),
      'build.sbt',
      {},
    );
...
function stubSubProcessExec(t) {
  const executeStub = sinon.stub(subProcess, 'execute').callsFake(() => {
