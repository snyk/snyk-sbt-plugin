...
test('parse `sbt dependencies` output: multi configuration', async (t) => {
  ...
  t.equal(depTree.name, 'testApp', 'package name');
  ...
  t.equal(
    ...
    '0.0.1',
    'resolved stand-alone dependency',
  );
  ...
  t.equal(
    ...
    '1.6.4',
    'resolved correct version for discovery',
  );
  ...
  t.equal(
    depSet.some((dep) => dep.includes('scala-library')),
    false,
    'output does NOT include scala-library',
  );
});
...
test('parse `sbt dependencies` output: single configuration', async (t) => {
  ...
  t.equal(
    depTree.dependencies!['com.google.code.gson:gson'].version,
    '2.6.2',
